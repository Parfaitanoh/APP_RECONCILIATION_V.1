...
class ifuturPayinProcessor:
    def __init__(self, data_file, partner_file):
        ...
        self._partner_label = "IFUTUR PAYIN"
    ...
    def process(self):
        ...
        select_taux=dfop.groupby(['payment_method','Taux(%)']).agg(
                Nombre=('amount', 'count'),
                Volume_transaction=('amount','sum')
            )
        ...
        dfop['Net'] = np.where(
            dfop['payment_method'] == 'nita_online',
            dfop['amount'] + ts['fees'],
            dfop['amount'])

        dfop_unique = dfop.drop_duplicates(subset='external_reference')
        ts= dfop_unique.loc[(dfop_unique['status'] == 'succeeded')]
        ts['Nombre']= ts['Net']
        ...
        correspondance_amount = dfop_unique.set_index('external_reference')['Net']
        # Utilisation de map pour ajouter les colonnes correspondantes à dfpmt
        dfpmt['Amountop'] = dfpmt['Transaction ID'].map(correspondance_amount)
